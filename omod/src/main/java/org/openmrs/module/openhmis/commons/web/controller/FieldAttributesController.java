...
import org.openmrs.module.openhmis.commons.api.compatibility.CompatibilityConstantsHolder;
...
public class FieldAttributesController extends AbstractFieldAttributesController {
	...
	@Autowired
	private CompatibilityConstantsHolder compatibilityConstantsHolder;
	...
	private String getLocations() {
		...
		if (hasPrivileges(compatibilityConstantsHolder.privilegeConstants.GET_LOCATIONS)) {
	...
	private String getDrugs() {
		SimpleObject results = new SimpleObject();
		if (hasPrivileges(compatibilityConstantsHolder.privilegeConstants.GET_CONCEPT_CLASSES)) {
	...
	private String getUsers() {
		SimpleObject results = new SimpleObject();
		if (hasPrivileges(compatibilityConstantsHolder.privilegeConstants.GET_USERS)) {
	...
	private String getProviders() {
		SimpleObject results = new SimpleObject();
		if (hasPrivileges(compatibilityConstantsHolder.privilegeConstants.GET_PROVIDERS)) {
	...
	private String getConcepts(String foreignKey) {
		SimpleObject results = new SimpleObject();
		if (hasPrivileges(compatibilityConstantsHolder.privilegeConstants.GET_CONCEPTS)) {
